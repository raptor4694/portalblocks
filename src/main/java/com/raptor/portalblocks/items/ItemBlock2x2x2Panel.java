...
public class ItemBlock2x2x2Panel extends ItemBlock {
	private String unlocalizedName;
	
	public ItemBlock2x2x2Panel(Block panelCorner) {
		super(panelCorner);
		BlockLargePanelCorner.class.cast(panelCorner);
	}
	...
	public String getUnlocalizedName() {
		return unlocalizedName == null? block.getUnlocalizedName() : "tile." + unlocalizedName;
	}
	...
	public EnumActionResult onItemUse(final EntityPlayer player, final World world, BlockPos pos, final EnumHand hand,
		...
		if(!player.isSneaking() && state.getBlock() instanceof BlockLargePanelCorner) {
			...
			case UP:
			case DOWN:
				System.out.println("hitX = " + hitX + "  hitZ = " + hitZ);
				if(hitX > 0.5f) {
					if(hitZ > 0.5f)
			...
			case WEST:
				...
				break;
			case EAST:
				System.out.println("hitY = " + hitY + "  hitZ = " + hitZ);
				if(hitY < 0.5f) {
					if(hitZ < 0.5f)
						type = EnumLargePanelCorner.BOTTOM_NORTH_WEST;
					else
		...
		System.out.println("pos = " + pos + "   type = " + type);

		bottomNorthWestCorner = pos.add(type.getOffsetToBottomNorthWest());
		System.out.println("bottom northwest corner 1 = " + bottomNorthWestCorner);
		if(!has2x2ReplaceableCube(world, player, face, itemstack, bottomNorthWestCorner)) {
			boolean flag = false;
			for(int i = 0; i < 3 && !flag; i++) {
				type = type.rotateAround(face);
				bottomNorthWestCorner = pos.add(type.getOffsetToBottomNorthWest());
				System.out.println("bottom northwest corner " + (i+2) + " = " + bottomNorthWestCorner);
		...
		for(EnumLargePanelCorner theType : EnumLargePanelCorner.values()) {
			...
			states[i] = state = this.block.getDefaultState().withProperty(TYPE, theType);
		...
		if(placed) {
			SoundType soundtype = this.block.getSoundType(state, world, pos2, player);

...
public class BlockSquarePanel extends BlockPanel implements IVariantHolder {
	...
	public EnumPanelFace getFaceType(EnumFacing face, IBlockState state, IBlockAccess world, BlockPos pos) {
		return state.getValue(VARIANT).getFaceType(portalable);
	}

	public static enum EnumSquarePanelType implements IStringSerializable {
		NORMAL(EnumPanelFace.WHITE_SQUARE, EnumPanelFace.BLACK_SQUARE), 
		GRID(EnumPanelFace.WHITE_GRID, EnumPanelFace.BLACK_GRID);
		...
		private final EnumPanelFace whiteType, blackType;
		
		private EnumSquarePanelType(EnumPanelFace whiteType, EnumPanelFace blackType) {
		...
		public EnumPanelFace getFaceType(boolean isPortalable) {
			return isPortalable? whiteType : blackType;
		}
